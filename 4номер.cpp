...
double getValue()
{
    double value = 0.0; // Инициализация добавлена
    cin >> value;
    if (cin.fail())
    {
 ...
 * Считывает значение шага с клавиатуры и проверяет, что оно положительное.
 * @return Положительное значение шага типа double.
 * @note Если введенное значение шага неположительное, программа завершается с кодом ошибки 1.
 */
double getPositiveStep()
{
    cout << "Введите шаг: ";
    double step = getValue(); // Инициализация через вызов getValue()
    if (step <= 0)
    {

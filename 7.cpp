...
/**
 * @brief Заполняет двумерный массив случайными значениями в диапазоне [-100, 100].
...
int main()
{
    ...
    fillArrayRandomly(originalArray, rows, cols);
...
void fillArrayRandomly(int** array, const int rows, const int cols)
{
    for (int i = 0; i < rows; ++i)
    {
        ...
        {
            array[i][j] = rand() % 201 - 100; // Диапазон [-100, 100]
        }
    }
}
